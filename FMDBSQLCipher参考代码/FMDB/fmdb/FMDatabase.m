...
//#define SETSQLKEY @"IMSqlCipherKey" 
...
- (void)upgradeDatabase:(NSString *)path{
    ...
    if(targetPath){
        const char *sqlQ = [[NSString stringWithFormat:@"ATTACH DATABASE '%@' AS encrypted KEY '%@';", targetPath, SETSQLKEY] UTF8String];
        ...
        if (sqlite3_open([path UTF8String], &unencrypted_DB) == SQLITE_OK) {
            ...
            if (errmsg) {
                NSLog(@"-------upgradeDatabase:%@", [NSString stringWithUTF8String:errmsg]);
